...
iPhoneのホーム画面からワンタップで、自作Discord Botに母親の個人DMへ「今日は夕食いらない」と送らせるパーソナルPWA。
...
[Discord Bot]
    ↓
[母親の個人DM]  ← 「夕食連絡Bot からの新着メッセージ」として届く
```
...
2. 招待リンクの設定画面が出るので、画面下の **「招待リンクを編集」** で:
   ...
   - 使用回数: **1回** (セキュリティのため)
3. **新しいリンクを生成** → リンクをコピー
4. リンクを既存の家族グループDMで送る
5. 母親が招待リンクをタップして「夕食連絡」サーバーに参加(Discordアプリ必須)

#### 1-5. 母親のUser IDを取得

1. 母親にサーバーへ入ってもらう前/後どちらでもOK、まず自分側のDiscordで:
2. Discord 設定(歯車) → **詳細設定** → **開発者モード** ON
3. 「夕食連絡」サーバーのメンバーリストで母親を右クリック → **IDをコピー**
4. これが `DISCORD_RECIPIENT_USER_ID` の値 (18桁前後の数字)

#### 1-6. お母様のDM受信設定を確認

Botからの送信が成功するには、母親側で以下が必要:
1. Discord 設定 → **プライバシー・安全** → 「サーバーメンバーからのダイレクトメッセージを許可する」が **ON** (デフォルトでON)
2. もしOFFになっている場合、Botからの送信は `Cannot send messages to this user` で失敗するので、母親に一度ONにしてもらう
...
母親のDiscordに「botテスト」というBotからのDMが届けば成功。
...
| `DISCORD_RECIPIENT_USER_ID` | 母親のUser ID | Step 1-5 |
...
`http://localhost:3000` をブラウザで開き、ボタンを押す → 母親のDiscordに「今日は夕食いらない」がBotからのDMで届けばOK
...
5. 母親のDiscordに「今日は夕食いらない」がBotからのDMで届くことを確認
...
- **App Icon**: ここの画像が母親のDM一覧に表示される
- **Application Name**: 母親のDMで表示されるBot名
...
- 万が一トークンが漏れた場合の被害範囲: Botが母親にDMを送れるだけ。家族グループDMや他チャンネルには影響しない (Botがいないため)
...
| 症状 | 原因と対処 |
|---|---|
| `failed to open DM channel: 403` | Botが母親と同じサーバーにいない → Step 1-3, 1-4 を確認 |
| `failed to send DM: 50007` (Cannot send messages to this user) | 母親のDM受信設定がOFF → Step 1-6 を確認 |
| `unauthorized` | `SHARED_TOKEN` と `VITE_SHARED_TOKEN` の値が一致していない |
| `server misconfigured` | Vercelの環境変数が登録されていない or デプロイ前 |
| Botからのメッセージが届かない (エラーは出ない) | 母親の通知設定でBotからのDMをミュートしている可能性 |
